...
////////////////////////////////////////////////////////////////
	case 'm':
	    model->texture_file = new char[sizeof(buf)];
	    fscanf(file, "%s", model->texture_file);
	    break;
///////////////////////////////////////////////////////////////
        case 'N':
...
////////////////////////////////////////////////////
    ...
    numvertices = numnormals = numtexcoords = numtriangles = numpolygons = 0;
    while(fscanf(file, "%s", buf) != EOF) {
        switch(buf[0]) {
        case '#':               /* comment */
            /* eat up rest of line */
...
/////////////////////////////////////////////
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'v':               /* v, vn, vt */
		    ...
		    while(1)
			{
				P(numpolygons).vindices[num] = v < 0 ? v + numvertices : v;
				P(numpolygons).nindices[num] = n < 0 ? n + numnormals : n;
				GLuint oldn = n, oldv = v;
				fscanf(file, "%d//%d", &v, &n);
				if(oldn == n && oldv == v) {P(numpolygons).numvertices = ++num; numpolygons++;break;}
				num++;
			}

                } else if (sscanf(buf, "%d/%d/%d", &v, &t, &n) /*== 3*/ > 1) {
                    /* v/t/n */
                    P(numpolygons).n = 1;
                    P(numpolygons).t = 1;
		    int num = 0;
		    while(1)
			{
				P(numpolygons).vindices[num] = v < 0 ? v + numvertices : v;
				P(numpolygons).nindices[num] = n < 0 ? n + numnormals : n;
				P(numpolygons).tindices[num] = t < 0 ? t + numtexcoords : t;
				GLuint oldv = v, oldt = t, oldn = n;
				fscanf(file, "%d/%d/%d", &v, &t, &n);
				if(oldv==v && oldt==t && oldn == n) {P(numpolygons).numvertices = ++num; numpolygons++;break;}
				num++;
			}
			
                } else if (sscanf(buf, "%d/%d", &v, &t) /*== 2*/ > 1) {
                    /* v/t */
                    P(numpolygons).t = 1;
		    int num = 0;
		    while(1)
			{
				P(numpolygons).vindices[num] = v < 0 ? v + numvertices : v;
				P(numpolygons).tindices[num] = t < 0 ? t + numtexcoords : t;
				GLuint oldv =  v, oldt = t;
				fscanf(file, "%d/%d", &v, &t);
				if(oldv == v && oldt == t) {P(numpolygons).numvertices = ++num; numpolygons++;break;}
				num++;
			}
		    ...
		    while(1)
			{
				...
				GLuint old = v;
				fscanf(file, "%d", &v);
				if(v == old) {P(numpolygons).numvertices = ++num; numpolygons++;break;}
				num++;
			}
...
glmReadOBJ(char* filename)
{
    ...
    model->triangles = (GLMtriangle*)malloc(sizeof(GLMtriangle) *
        model->numtriangles);
////////////////////////////////////////////////////////////////////
    model->polygons = (GLMpolygon*)malloc(sizeof(GLMpolygon) * model->numpolygons);
    int num = model->numpolygons, i = 0;
    while(num-- > 0)
    {
		(model->polygons[i]).vindices = (GLuint*)malloc(sizeof(GLuint) * (int)((float)model->numvertices+ 1));
		(model->polygons[i]).nindices = (GLuint*)malloc(sizeof(GLuint) * (int)((float)model->numnormals + 1));
		(model->polygons[i++]).tindices = (GLuint*)malloc(sizeof(GLuint) * (int)((float)model->numtexcoords + 1));
		model->polygons[i].n = 0; model->polygons[i].t = 0;
    }
////////////////////////////////////////////////////////////////////
    if (model->numnormals) {
